...
namespace
{
    ...
    static float analyzerReleaseCoeffForSpeed (int speedIndex) noexcept
    { switch (speedIndex) { case 0: return 0.40f; case 1: return 0.20f; case 2: return 0.10f; case 3: return 0.04f; case 4: return 0.015f; default: return 0.04f; } }
}
...
void D7SChannelStripFullAudioProcessor::pushPreSpectrumSample (float sample) noexcept
{ preSpectrumFifo[(size_t) preSpectrumFifoIndex++] = sample; if (preSpectrumFifoIndex >= spectrumFFTSize) { runPreSpectrumFFT(); preSpectrumFifoIndex = 0; } }

void D7SChannelStripFullAudioProcessor::pushPostSpectrumSample (float sample) noexcept
{ postSpectrumFifo[(size_t) postSpectrumFifoIndex++] = sample; if (postSpectrumFifoIndex >= spectrumFFTSize) { runPostSpectrumFFT(); postSpectrumFifoIndex = 0; } }
...
void D7SChannelStripFullAudioProcessor::runPreSpectrumFFT() noexcept
{
    ...
    {
        const float f0 = logFrequencyForBin (band, numSpectrumBins); const float f1 = logFrequencyForBin (juce::jmin (band + 1, numSpectrumBins - 1), numSpectrumBins);
        const int bin0 = juce::jlimit (1, fftBins - 1, (int) std::floor ((f0 / nyquist) * (float) fftBins)); const int bin1 = juce::jlimit (bin0 + 1, fftBins, (int) std::ceil ((f1 / nyquist) * (float) fftBins));
        float energy = 0.0f; int count = 0; for (int bin = bin0; bin < bin1; ++bin) { const float mag = preSpectrumFftData[(size_t) bin] / (float) spectrumFFTSize; energy += mag * mag; ++count; }
        const float rms = std::sqrt (energy / (float) juce::jmax (1, count)); float targetDb = juce::Decibels::gainToDecibels (rms, -120.0f) + 42.0f; targetDb = juce::jlimit (-120.0f, 24.0f, targetDb);
        auto& smooth = preSpectrumSmoothedDb[(size_t) band]; smooth += (targetDb > smooth ? 0.55f : release) * (targetDb - smooth);
        auto& peak = preSpectrumPeakDb[(size_t) band]; if (smooth > peak) peak = smooth; else peak -= 0.40f;
        analyzerState.preDb[(size_t) band].store (smooth); analyzerState.prePeakDb[(size_t) band].store (peak);
    }
}
...
void D7SChannelStripFullAudioProcessor::runPostSpectrumFFT() noexcept
{
    ...
    for (int band = 0; band < numSpectrumBins; ++band)
    {
        const float f0 = logFrequencyForBin (band, numSpectrumBins); const float f1 = logFrequencyForBin (juce::jmin (band + 1, numSpectrumBins - 1), numSpectrumBins);
        const int bin0 = juce::jlimit (1, fftBins - 1, (int) std::floor ((f0 / nyquist) * (float) fftBins)); const int bin1 = juce::jlimit (bin0 + 1, fftBins, (int) std::ceil ((f1 / nyquist) * (float) fftBins));
        float energy = 0.0f; int count = 0; for (int bin = bin0; bin < bin1; ++bin) { const float mag = postSpectrumFftData[(size_t) bin] / (float) spectrumFFTSize; energy += mag * mag; ++count; }
        const float rms = std::sqrt (energy / (float) juce::jmax (1, count)); float targetDb = juce::Decibels::gainToDecibels (rms, -120.0f) + 42.0f; targetDb = juce::jlimit (-120.0f, 24.0f, targetDb);
        auto& smooth = postSpectrumSmoothedDb[(size_t) band]; smooth += (targetDb > smooth ? 0.55f : release) * (targetDb - smooth);
        auto& peak = postSpectrumPeakDb[(size_t) band]; if (smooth > peak) peak = smooth; else peak -= 0.40f;
        analyzerState.postDb[(size_t) band].store (smooth); analyzerState.postPeakDb[(size_t) band].store (peak); spectrumBinsDb[(size_t) band].store (smooth);
    }
}
...
void D7SChannelStripFullAudioProcessor::processAudioBlock (juce::AudioBuffer<FloatType>& buffer)
{
    ...
    const int numCh = buffer.getNumChannels(); const int n = buffer.getNumSamples(); const double rackIn = juce::Decibels::decibelsToGain ((double) readAtomic (rackInputParam, 0.0f)); const double rackOut = juce::Decibels::decibelsToGain ((double) readAtomic (rackOutputParam, 0.0f)); const double rackMix = readAtomic (rackMixParam, 100.0f) / 100.0; double inputPeak = 0.0, outputPeak = 0.0; juce::AudioBuffer<FloatType> rackDry (numCh, n);
    for (int ch = 0; ch < numCh; ++ch) { auto* data = buffer.getWritePointer (ch); auto* dry = rackDry.getWritePointer (ch); for (int i = 0; i < n; ++i) { data[i] = (FloatType) ((double) data[i] * rackIn); dry[i] = data[i]; inputPeak = juce::jmax (inputPeak, std::abs ((double) data[i])); if (ch == 0) pushPreSpectrumSample ((float) data[i]); } }
    double bpm = 120.0; if (auto* playHead = getPlayHead()) { juce::AudioPlayHead::CurrentPositionInfo info; if (playHead->getCurrentPosition (info) && info.bpm > 0.0) bpm = info.bpm; }
    const auto order = getModuleOrder(); for (int slot = 0; slot < numRackModules; ++slot) processModuleById (order[(size_t) slot], buffer, bpm);
    for (int ch = 0; ch < numCh; ++ch) { auto* data = buffer.getWritePointer (ch); auto* dry = rackDry.getReadPointer (ch); for (int i = 0; i < n; ++i) { double x = (double) dry[i] * (1.0 - rackMix) + (double) data[i] * rackMix; x *= rackOut; outputPeak = juce::jmax (outputPeak, std::abs (x)); data[i] = (FloatType) juce::jlimit (-4.0, 4.0, x); if (ch == 0) pushPostSpectrumSample ((float) x); } }
...
void D7SChannelStripFullAudioProcessor::getStateInformation (juce::MemoryBlock& destData) { auto state = apvts.copyState(); std::unique_ptr<juce::XmlElement> xml (state.createXml()); if (xml != nullptr) copyXmlToBinary (*xml, destData); }
void D7SChannelStripFullAudioProcessor::setStateInformation (const void* data, int sizeInBytes) { std::unique_ptr<juce::XmlElement> xmlState (getXmlFromBinary (data, sizeInBytes)); if (xmlState != nullptr) { auto state = juce::ValueTree::fromXml (*xmlState); if (state.isValid()) apvts.replaceState (state); } }
