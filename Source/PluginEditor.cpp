...
void D7SChannelStripFullAudioProcessorEditor::syncChoiceButtons()
{
    ...
    sync (delayTimeButtonsA, "delay_time", 0);
    sync (delayTimeButtonsB, "delay_time", 5);
...
void D7SChannelStripFullAudioProcessorEditor::syncRackVisuals()
{
    ...
    syncOne (comp2a, "comp2a_bypass"); syncOne (tube, "tube_bypass"); syncOne (esser, "esser_bypass"); syncOne (delay, "delay_bypass");
}
...
void D7SChannelStripFullAudioProcessorEditor::installModuleDragHandlers()
{
    auto install = [this] (RackModuleComponent& module)
    {
        ...
        module.onDragMove = [this] (RackModuleComponent* m, const juce::MouseEvent& e)
        {
            ...
            if (targetSlot >= 0) moveModuleToSlot (moduleId, targetSlot);
        };
    ...
    install (noiseGate); install (eq4k); install (comp76); install (comp2a); install (tube); install (esser); install (delay);
}
...
int D7SChannelStripFullAudioProcessorEditor::getSlotForMousePosition (juce::Point<int> contentPoint) const noexcept
{
    for (int i = 0; i < D7SChannelStripFullAudioProcessor::numRackModules; ++i)
        if (moduleSlotBounds[(size_t) i].expanded (8, 120).contains (contentPoint)) return i;
    for (int i = 0; i < D7SChannelStripFullAudioProcessor::numRackModules; ++i)
        if (contentPoint.x < moduleSlotBounds[(size_t) i].getCentreX()) return i;
...
void D7SChannelStripFullAudioProcessorEditor::moveModuleToSlot (int moduleId, int targetSlot)
{
    ...
    if (currentSlot < targetSlot)
        for (int i = currentSlot; i < targetSlot; ++i) editorModuleOrder[(size_t) i] = editorModuleOrder[(size_t) i + 1];
    else
        for (int i = currentSlot; i > targetSlot; --i) editorModuleOrder[(size_t) i] = editorModuleOrder[(size_t) i - 1];
    editorModuleOrder[(size_t) targetSlot] = moduleId;
    commitModuleOrderToProcessor(); resized();
}
...
void D7SChannelStripFullAudioProcessorEditor::resized()
{
    ...
    auto getPanelWidth = [] (int moduleId)
    {
        switch (moduleId)
        {
            ...
            case D7SChannelStripFullAudioProcessor::moduleDelay:    return 320;
            default: return 220;
        }
    };
    ...
    auto layoutModuleById = [&] (int moduleId, juce::Rectangle<int> panel)
    {
        switch (moduleId)
        {
            case D7SChannelStripFullAudioProcessor::moduleNoiseGT1:
            {
                ...
                noiseMeterLabel.setBounds (panel.removeFromTop (20)); noiseGrMeter.setBounds (panel.removeFromTop (16));
                break;
            }
            case D7SChannelStripFullAudioProcessor::moduleEQ4K:
            {
                panelHeader (panel, eq4k);
                auto filterRow = panel.removeFromTop (105); layoutKnobLocal (eqHpf, nextRowLeft (filterRow)); layoutKnobLocal (eqLpf, nextRowLeft (filterRow)); eqBypassButton.setBounds (filterRow.removeFromLeft (90).withHeight (28)); panel.removeFromTop (8);
                auto hfRow = panel.removeFromTop (105); layoutKnobLocal (eqHfGain, nextRowLeft (hfRow)); layoutKnobLocal (eqHfFreq, nextRowLeft (hfRow)); eqHfBellButton.setBounds (hfRow.removeFromLeft (90).withHeight (28)); panel.removeFromTop (8);
                auto hmfRow = panel.removeFromTop (105); layoutKnobLocal (eqHmfGain, nextRowLeft (hmfRow)); layoutKnobLocal (eqHmfFreq, nextRowLeft (hmfRow)); layoutKnobLocal (eqHmfQ, nextRowLeft (hmfRow)); panel.removeFromTop (8);
                auto lmfRow = panel.removeFromTop (105); layoutKnobLocal (eqLmfGain, nextRowLeft (lmfRow)); layoutKnobLocal (eqLmfFreq, nextRowLeft (lmfRow)); layoutKnobLocal (eqLmfQ, nextRowLeft (lmfRow)); panel.removeFromTop (8);
                auto lfRow = panel.removeFromTop (105); layoutKnobLocal (eqLfGain, nextRowLeft (lfRow)); layoutKnobLocal (eqLfFreq, nextRowLeft (lfRow)); eqLfBellButton.setBounds (lfRow.removeFromLeft (90).withHeight (28));
                break;
            }
            case D7SChannelStripFullAudioProcessor::module76:
            {
                panelHeader (panel, comp76);
                auto c76Top = panel.removeFromTop (105); layoutKnobLocal (comp76Input, nextRowLeft (c76Top)); layoutKnobLocal (comp76Output, nextRowLeft (c76Top));
                auto c76Mid = panel.removeFromTop (105); layoutKnobLocal (comp76Attack, nextRowLeft (c76Mid)); layoutKnobLocal (comp76Release, nextRowLeft (c76Mid));
                comp76BypassButton.setBounds (panel.removeFromTop (28)); panel.removeFromTop (8);
                auto ratioArea = panel.removeFromTop (30); for (auto& b : comp76RatioButtons) { b.setBounds (ratioArea.removeFromLeft (40)); ratioArea.removeFromLeft (4); }
                panel.removeFromTop (8); comp76MeterLabel.setBounds (panel.removeFromTop (20)); comp76GrMeter.setBounds (panel.removeFromTop (16));
                break;
            }
            case D7SChannelStripFullAudioProcessor::module2A:
            {
                panelHeader (panel, comp2a);
                auto c2aTop = panel.removeFromTop (105); layoutKnobLocal (comp2aPeak, nextRowLeft (c2aTop)); layoutKnobLocal (comp2aGain, nextRowLeft (c2aTop));
                auto c2aMid = panel.removeFromTop (105); layoutKnobLocal (comp2aEmphasis, nextRowLeft (c2aMid)); layoutKnobLocal (comp2aMix, nextRowLeft (c2aMid));
                ...
                panel.removeFromTop (8); comp2aMeterLabel.setBounds (panel.removeFromTop (20)); comp2aGrMeter.setBounds (panel.removeFromTop (16));
                break;
            }
            case D7SChannelStripFullAudioProcessor::moduleTube:
            {
                panelHeader (panel, tube);
                auto tubeTop = panel.removeFromTop (105); layoutKnobLocal (tubeBeauty, nextRowLeft (tubeTop)); layoutKnobLocal (tubeBeast, nextRowLeft (tubeTop));
                auto tubeMid = panel.removeFromTop (105); layoutKnobLocal (tubeSensitivity, nextRowLeft (tubeMid)); layoutKnobLocal (tubeMix, nextRowLeft (tubeMid));
            ...
            case D7SChannelStripFullAudioProcessor::moduleEsser:
            {
                ...
                auto esTop = panel.removeFromTop (105); layoutKnobLocal (esserThreshold, nextRowLeft (esTop)); layoutKnobLocal (esserFreq, nextRowLeft (esTop));
                auto esMid = panel.removeFromTop (105); layoutKnobLocal (esserRange, nextRowLeft (esMid)); esserBypassButton.setBounds (esMid.removeFromLeft (120).withHeight (28));
                ...
                panel.removeFromTop (8); esserMeterLabel.setBounds (panel.removeFromTop (20)); esserGrMeter.setBounds (panel.removeFromTop (16));
                break;
            }
            case D7SChannelStripFullAudioProcessor::moduleDelay:
            {
                panelHeader (panel, delay);
                auto dTop = panel.removeFromTop (105); layoutKnobLocal (delayMix, nextRowLeft (dTop)); layoutKnobLocal (delayFeedback, nextRowLeft (dTop)); layoutKnobLocal (delayGlideTime, nextRowLeft (dTop));
                delayBypassButton.setBounds (panel.removeFromTop (28)); panel.removeFromTop (6);
                auto timeA = panel.removeFromTop (28); for (auto& b : delayTimeButtonsA) { b.setBounds (timeA.removeFromLeft (52)); timeA.removeFromLeft (4); }
                auto timeB = panel.removeFromTop (28); for (auto& b : delayTimeButtonsB) { b.setBounds (timeB.removeFromLeft (52)); timeB.removeFromLeft (4); }
                panel.removeFromTop (6);
                auto glideRow = panel.removeFromTop (28); delayGlideButton.setBounds (glideRow.removeFromLeft (72)); glideRow.removeFromLeft (8); for (auto& b : delayDirectionButtons) { b.setBounds (glideRow.removeFromLeft (70)); glideRow.removeFromLeft (4); }
                break;
            }
            default: break;
        }
