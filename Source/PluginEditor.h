...
private:
    ...
    public:
        ...
        void mouseWheelMove (const juce::MouseEvent& e, const juce::MouseWheelDetails& wheel) override
        {
            ...
            {
                auto horizontal = wheel; horizontal.deltaX = wheel.deltaY; horizontal.deltaY = 0.0f;
                juce::Viewport::mouseWheelMove (e, horizontal); repaint(); return;
            }
            juce::Viewport::mouseWheelMove (e, wheel); repaint();
        }
        ...
        {
            auto* viewed = getViewedComponent(); if (viewed == nullptr) return;
            ...
            auto drawArrow = [&] (bool left)
            {
                ...
                juce::Path p;
                if (left) p.addTriangle (x + 5.0f, y - 10.0f, x - 5.0f, y, x + 5.0f, y + 10.0f);
                else p.addTriangle (x - 5.0f, y - 10.0f, x + 5.0f, y, x - 5.0f, y + 10.0f);
                g.setColour (juce::Colour (255, 180, 60).withAlpha (0.42f)); g.fillPath (p);
            };
            if (canLeft) drawArrow (true); if (canRight) drawArrow (false);
        }
    ...
    std::array<int, D7SChannelStripFullAudioProcessor::numRackModules> editorModuleOrder {
        ...
        D7SChannelStripFullAudioProcessor::moduleClipper,
    ...
    RackModuleComponent clipper   { "D7S Clipper" };
    ...
    juce::ToggleButton eqLfBellButton; juce::ToggleButton eqHfBellButton; juce::ToggleButton eqBypassButton; std::unique_ptr<ButtonAttachment> eqLfBellAttachment; std::unique_ptr<ButtonAttachment> eqHfBellAttachment; std::unique_ptr<ButtonAttachment> eqBypassAttachment;
    ...
    ParamSlider clipperDrive; ParamSlider clipperCeiling; ParamSlider clipperShape; ParamSlider clipperMix; juce::ToggleButton clipperBypassButton; std::unique_ptr<ButtonAttachment> clipperBypassAttachment;
